...
def write_voxel(voxel_model, filename):
    with open(filename, 'wb') as fp:
        ...
        if voxel_model.data.dtype != np.uint8:
            voxel_model.data = voxel_model.data.astype(np.uint8)
        # if not voxel_model.data.flags['C_CONTIGUOUS']:
        #     voxel_model = np.ascontiguousarray(voxel_model, dtype=np.uint8)
        cutils.get_state_ctr(voxel_model.data ,state_ctr)
        ...
        c = 0
        while c < state_ctr.shape[0]:
            write_pair(fp, state_ctr[c,0], state_ctr[c,1])
            c += 1
            if c >= state_ctr.shape[0] or state_ctr[c,0]==2: break